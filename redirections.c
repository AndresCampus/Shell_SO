...
int main(int argc, char *argv[]) {
  ...
  // redireccionamos la entrada estandar (stdin) al fichero que acabamos de
  // abrir (infile)
  fnum1 = fileno(infile);
  fnum2 = fileno(stdin);
  ...
  // Cerramos el original. Importante: dup2 ha duplicado el descriptor al stdin
  // (0), por lo que el descriptor original (fnum1) ya no es necesario.
  fclose(infile);
  ...
  // redireccionamos la salida estandar (stdout) al fichero que acabamos de
  // abrir (outfile)
  ...
  // Idem para la salida. Una vez redireccionado el stdout (1), cerramos el
  // puntero original para evitar fugas de descriptores de fichero.
  fclose(outfile);
